...
export default function TrendingTable({ isDarkMode }: TrendingTableProps) {
  ...
  return (
    <section className={`trending-table glass ounded-xl ${isDarkMode ? 'bg-opacity-50' : 'bg-opacity-30'} backdrop-blur-md`}>
      ...
      <table className="w-full border-separate border-spacing-2">
        ...
        <tbody>
          {trendingProfiles.map((profile, index) => {
            // Local state for each row to handle hover and selected ratings
            ...
            return (
              <tr key={index} className={` ${isDarkMode ? 'bg-card-bg-dark' : 'bg-card-bg-light'} border rounded-lg text-center align-middle`}>
