...
export default function AuthForm({ isDarkMode }: { isDarkMode: boolean }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Step 1: Pre-create placeholder profile
      ...
      profilePlaceholder = data;
  
      const shortId = profilePlaceholder.short_id;
  
      // Step 2: Upload images
      const profileImageUrl = await uploadImageToBucket(profilePicture, shortId, "profile.jpg");
      const bannerImageUrl = await uploadImageToBucket(bannerImage, shortId, "banner.jpg");
  
      if (!profileImageUrl || !bannerImageUrl) {
        throw new Error("Image URLs could not be retrieved.");
      }
  
      // Step 3: Update profile with image URLs
      ...
      // Rollback: Delete the placeholder profile if creation fails
